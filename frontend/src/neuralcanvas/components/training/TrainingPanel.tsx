...
export function TrainingPanel({ open: isOpen, onClose, nodes, edges, compact }: TrainingPanelProps) {
  ...
  useEffect(() => {
    ...
    fetchDatasets()
      .then((list) => {
        if (!cancelled) { setDatasets(list); setDatasetError(null); }
      })
      .catch((e) => {
        if (!cancelled) {
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-5">
        ...
        {datasetError && (
          <div className="rounded-xl bg-[var(--danger-muted)] border border-[var(--danger)]/30 text-[var(--danger)] text-sm p-3">
            <p>{datasetError}</p>
            <p className="text-[var(--foreground-muted)] text-xs mt-1">Is the backend running on port 8000?</p>
          </div>
        )}
        ...
        <div>
          <div className="flex items-center gap-2 mb-2">
            <Database className="h-3.5 w-3.5 text-[var(--foreground-muted)]" />
            <label className="text-sm font-medium text-[var(--foreground)]">Dataset</label>
          </div>
          <select
            id="training-dataset"
        ...
        <div className="flex gap-2">
          <button
            type="button"
            onClick={handleStart}
            disabled={!!graphError || isTraining}
